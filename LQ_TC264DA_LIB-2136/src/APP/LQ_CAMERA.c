...
unsigned char Image_Temp[LCDH][LCDW];
...
void Get_Use_Image(void)
{
    ...
    for (i = 20; i < LCDH+20; i++)          //神眼高 120 / 2  = 60，
    // for (i = 0; i < IMAGEH; i += 3)       //OV7725高 240 / 3  = 80，
    {
        for (j = 45; j <= LCDW+45; j++)     //神眼宽188 / 2  = 94，
        // for (j = 0; j <= IMAGEW; j += 3)  //OV7725宽320 / 3  = 106，
        {
            Image_Use[row][line] = Image_Data[i][j];
            line++;
...
*/
    ...
    for (i = 0; i < LCDH; i++)
    {
        for (j = 0; j < LCDW; j++)
        {
            if (Image_Use[i][j] > Threshold+20) //数值越大，显示的内容越多，较浅的图像也能显示出来
                Bin_Image[i][j] = 255;
            else
                Bin_Image[i][j] = 0;
        }
    ...
    for (nr = 1; nr < LCDH - 1; nr++)
          {
              for (nc = 1; nc < LCDW - 1; nc = nc + 1)
              {
                  Image_Temp[nr][nc]=Bin_Image[nr][nc];
              }
          }
}
